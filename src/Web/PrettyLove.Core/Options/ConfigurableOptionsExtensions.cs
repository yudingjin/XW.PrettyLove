...
{
    public static class ConfigurableOptionsExtensions
    {
        /// <summary>
        /// 所有配置项注册
        /// </summary>
        /// <param name="services"></param>
        public static void AddAllOptionsRegister(this IServiceCollection services)
        {
            if (services == null) throw new ArgumentNullException(nameof(services));

            var list = typeof(ConfigurableOptionsExtensions).Assembly.GetTypes().Where(s => !s.IsInterface && typeof(IConfigurableOptions).IsAssignableFrom(s));
            foreach (var optionType in list)
            {
